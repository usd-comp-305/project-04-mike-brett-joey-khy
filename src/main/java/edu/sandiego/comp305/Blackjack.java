...
public class Blackjack implements Game {
    ...
    public Blackjack(final Scanner scanner) {
        this.scanner = scanner;
    }


    private void hit(final ArrayList<Card> hand) {
    ...
    private void doubleDown(final ArrayList<Card> hand) {
        betAmount *= 2;
        hand.add(DeckOfCards.dealCard(deck));
        playerStands = true;
    }
    ...
    public void playGame() {
        ...
        final int result = handleBet(betAmount);
    ...
    void playerDecisions(final ArrayList<Card> hand,
                         final int handTotal) {
        int currentHandTotal = handTotal;
        while (currentHandTotal < BLACKJACK && !playerStands) {
            System.out.println("You have a " + currentHandTotal +
                    " with a " + hand.getFirst().getFaceValue() +
                    " and a " + hand.getLast().getFaceValue() +
                    " The dealer is showing a " +
                    dealerFaceUpCard.getFaceValue() +
                    " what would you like to do?");

            final int userDecision = scanner.nextInt();

            if (userDecision == HIT) {
                hit(hand);
                currentHandTotal = calculateHandTotal(hand);
            } else if (userDecision == STAND) {
                playerStands = true;
            } else if (userDecision == DOUBLE && hand.size()
                    == BLACKJACK_HAND_SIZE && !hasSplit) {
                doubleDown(hand);
                currentHandTotal = calculateHandTotal(hand);
            } else if (userDecision == SPLIT) {
                if (hand.getFirst().getFaceValue() ==
                        hand.getLast().getFaceValue() && !hasSplit) {
                    hasSplit = true;
                    split();
                    currentHandTotal = calculateHandTotal(hand);
                } else {
                    System.out.println("Cards must be the " +
                            "same to split, try again");
                }
            } else {
                System.out.println("Invalid input, Enter 1 to hit, " +
                        "2 to stand, 3 to double, and 4 to split");
            }
        }
    }
    ...
    void dealerDecisions() {
        ...
        System.out.println("Dealer shows a " +
                dealerHand.getLast().getFaceValue() +
    ...
    int calculateHandTotal(final ArrayList<Card> hand) {
        int total = 0;
        int numAces = 0;

        for (Card card : hand) {
            final int cardValue = card.getFaceValue().getCardValue();
    ...
    public int handleBet(final int bet) {
        int winAmount = 0;

        if (hasSplit) {
            if (splitTotal > BLACKJACK) {
                winAmount -= bet;
            } else if (splitTotal == BLACKJACK &&
                    splitHand.size() == BLACKJACK_HAND_SIZE) {
        ...
        if (playerTotal > BLACKJACK) {
            winAmount -= bet;
                ...
                playerHand.size() == BLACKJACK_HAND_SIZE) {
            winAmount += (int) (bet * BLACKJACK_PAYOUT_MULTIPLIER);
        } else if (dealerTotal > BLACKJACK) {
            winAmount += bet;
    ...
    void setDeck(final ArrayList<Card> deck) {
        this.deck = deck; }

    void setPlayerTotal(final int total) {
        this.playerTotal = total; }

    void setDealerTotal(final int total) {
        this.dealerTotal = total; }

    void setSplitTotal(final int total) {
        this.splitTotal = total; }

    void setHasSplit(final boolean hasSplit) {
